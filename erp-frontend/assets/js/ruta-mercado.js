...
// ==============================================================================
// NAVEGACI√ìN TIPO EXCEL EN LA TABLA
// ==============================================================================
...
function navegarTabla(e, filaIdx, campo) {
    ...
    // Mapeo de campo l√≥gico a √≠ndice de <td> dentro del <tr>
    ...
    if (e.key === 'ArrowRight') {
        const orden = [0, 1, 2, 4]; // producto ‚Üí salida ‚Üí entrada ‚Üí precio
...
function renderizarTablaProductos() {
    ...
    if (filasProducto.length === 0) {
        tbody.innerHTML = `<tr id="fila-vacia"><td colspan="9" class="text-muted text-center py-4">
            <i class="bi bi-inbox fs-2 d-block mb-2"></i>Genera un Excel o a√±ade productos manualmente</td></tr>`;
        return;
    }
            ...
            <td style="position:relative;">
                ${fila.presentacion_id ? 
                    ...
                    `<input type="text" class="form-control form-control-sm" id="buscar-prod-${index}" 
                        placeholder="Escribe nombre o c√≥digo..." autocomplete="off"
...
function agregarPago() {
    ...
    // Inicializar label seg˙n el primer mÈtodo del select
...
function recalcularCuadre() {
    ...
    // Pagos (convertidos a Moneda Principal seg˙n el mÈtodo)
    ...
    // Cr√©ditos (input YA est√° en Moneda Principal, no se convierte)
...
function construirPayload() {
    ...
    const creditosPayload = creditos
        ...
        .map(c => ({
            ...
            monto_bs: (parseFloat(c.monto_principal) * tasa).toFixed(2), // Principal ‚Üí Secundaria (compatibilidad backend)
            descripcion: ''
        }));
...
async function cargarRutaExistente(id) {
    try {
        ...
        // 7. Cr√©ditos (backend viene en secundaria ‚Üí convertir a Principal para el input)
        ...
        // 9. Cobranzas (backend viene en secundaria ‚Üí convertir a Principal para edici√≥n)
    ...
    try {
        ...
        let html = `
            <div class="row mb-3">
                ...
                <div class="col-md-6"><strong>Observaci√≥n:</strong> ${r.observacion || 'N/A'}</div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-md-4">
                    <h6 class="fw-bold text-warning">Cr√©ditos</h6>
                    <ul class="list-group list-group-flush">
                        ${(r.creditos || []).map(c => `<li class="list-group-item py-1">${c.cliente_nombre}: ${monedaSecundaria} ${c.monto_bs}</li>`).join('') || '<li class="list-group-item py-1 text-muted">Sin cr√©ditos</li>'}
                    </ul>
                </div>
                <div class="col-md-4">
