...
class ClienteDetalleHistorialAPIView(APIView):
    ...
    def get(self, request, pk):
        try:
            ...
            # ═══════════════════════════════════════════════════════
            # PASO 1: Construir lista UNIFICADA de "ventas" (reales + deudas iniciales)
            # ═══════════════════════════════════════════════════════
            ventas_unificadas = []

            # 1A. Deudas iniciales (CxC sin venta asociada)
            ...
            for cxc_ini in deudas_iniciales:
                ventas_unificadas.append({
                    ...
                    "cxc_id": cxc_ini.id,             # ID real de la CxC
                    "fecha": cxc_ini.fecha_vencimiento if cxc_ini.fecha_vencimiento else timezone.now(),
                    "tipo": "DEUDA_INICIAL",
            ...
            # 1C. Ordenar todo junto por fecha descendente
            ventas_unificadas.sort(key=lambda x: x['fecha'] if x['fecha'] else timezone.now(), reverse=True)

            # ═══════════════════════════════════════════════════════
            # PASO 2: Construir lista de pagos (igual que antes + flag útil)
            # ═══════════════════════════════════════════════════════
            pagos_data = []
            for p in pagos:
                pagos_data.append({
                    ...
                    "cxc_id": p.cuenta.id,
                    "es_abono_deuda_inicial": p.cuenta.venta is None
                })

            # ═══════════════════════════════════════════════════════
            # PASO 3: Return con MISMAS CLAVES que antes (cero breaking changes)
            # ═══════════════════════════════════════════════════════
            return Response({
                ...
                "ventas": ventas_unificadas,   # <-- MISMA CLAVE, ahora incluye deudas iniciales
